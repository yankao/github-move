...
#
Test-AzResourceGroupDeployment -TemplateParameterFile "..\AzureDeployParameters\CAPAZISQLC01.parameters.json" -TemplateFile "..\AzureDeployTemplates\virtual_machine_NICs.v2.4.deploy.json" -ResourceGroupName ((dir).DirectoryName.Split("\")[-3]).TrimStart("AZ_").Replace("_Solution","") -Verbose -ErrorAction Stop

New-AzResourceGroupDeployment -TemplateParameterFile "..\AzureDeployParameters\CAPAZISQLC01.parameters.json" -TemplateFile "..\AzureDeployTemplates\virtual_machine_NICs.v2.4.deploy.json" -ResourceGroupName ((dir).DirectoryName.Split("\")[-3]).TrimStart("AZ_").Replace("_Solution","") -Verbose -ErrorAction Stop
