...
#
Test-AzResourceGroupDeployment -TemplateParameterFile "..\AzureDeployParameters\CAPAZIDSCC02.parameters.json" -TemplateFile "..\AzureDeployTemplates\virtual_machine_NICs.v2.2.deploy.json" -ResourceGroupName ((dir).DirectoryName.Split("\")[-3]).TrimStart("AZ_").Replace("_Solution","") -Verbose -ErrorAction Stop

New-AzResourceGroupDeployment -TemplateParameterFile "..\AzureDeployParameters\CAPAZIDSCC02.parameters.json" -TemplateFile "..\AzureDeployTemplates\virtual_machine_NICs.v2.2.deploy.json" -ResourceGroupName ((dir).DirectoryName.Split("\")[-3]).TrimStart("AZ_").Replace("_Solution","") -Verbose -ErrorAction Stop
